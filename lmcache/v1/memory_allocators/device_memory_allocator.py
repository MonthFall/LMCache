...
class DeviceMemoryAllocator(MemoryAllocatorInterface):
    """Allocates memory from a pre-allocated device memory pool.

    Vendor-neutral: works with any torch backend (CUDA, HIP, XPU, NPU).
    Pre-allocates a contiguous ``torch.uint8`` tensor on the specified device
    and manages it via an inner slab allocator (``TensorMemoryAllocator``)
    or paged allocator (``PagedTensorMemoryAllocator``).

    This is the device-generic counterpart of :class:`GPUMemoryAllocator`.
    New vendor-neutral allocators (e.g. :class:`PhxDeviceMemoryAllocator`)
    should inherit from this class rather than from ``GPUMemoryAllocator``.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        :param int size: The size of the device memory pool in bytes.
    ...
    def allocate(
        ...
        """Allocate one device-backed memory object.
    ...
    def batched_allocate(
        ...
        """Allocate multiple device-backed memory objects.
    ...
    def free(self, memory_obj: MemoryObj, allocator_type: Optional[str] = None) -> None:
        """Free one device-backed memory object.
    ...
    def batched_free(
        ...
        """Free multiple device-backed memory objects.
